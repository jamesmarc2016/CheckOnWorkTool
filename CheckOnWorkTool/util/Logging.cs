 ...
 * 日志记录类
 */
namespace CheckOnWork.Controller
{
    ...
    public class Logging
    {
        ...
        public static bool OpenLogFile()
        {
            try
            {
                ...
                //获取主程序运行目录下的log目录
                string curpath = Path.Combine(System.Windows.Forms.Application.StartupPath, CkeckOnWork.@Util.Constants.LOGGING_FILE_DIR);// Path.GetFullPath(".");//Path.GetTempPath();
                LogFilePath = curpath;
                ...
                //日志记录日期
                string new_date = DateTime.Now.ToString(CkeckOnWork.Util.Constants.DATE_FORMAT_YYYYMMDD);
                //日志文件名
                LogFileName = CkeckOnWork.Util.Constants.LOGGING_FILE_PREFIX + new_date + CkeckOnWork.Util.Constants.LOGGING_FILE_POSTFIX;
        ...
        {
            if (DateTime.Now.ToString(CkeckOnWork.Util.Constants.DATE_FORMAT_YYYYMMDD) != date)
            {
                lock (_lock)
                {
                    if (DateTime.Now.ToString(CkeckOnWork.Util.Constants.DATE_FORMAT_YYYYMMDD) != date)
